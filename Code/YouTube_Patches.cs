    ...
    public class YouTube_Patches
    {
                    ...
                    (
                        ...
                        for (int i = 0; i <= 0; i++)
                        {
                                ...
                                }.All(smaliProperties.Full.Contains))
                            {
                                ...
                                {
                                    if (smaliProperties.Lines[j].Contains(smaliSearchKeys[i]))
                                    {
                                        (int, string[]) patch = (
                                            j,
        ...
        public static List<bool> Non_Root_MicroG()
        {
            return [
                ...
                new SmaliUtils.SubPatchModule<SmaliUtils.StringTransform[]>(
                    [
                        new("</PreferenceScreen>", "</PreferenceScreen>", $"<PreferenceCategory android:icon=\"@drawable/microg_settings_key_icon\" android:persistent=\"false\" android:layout=\"@layout/preference_group_title\" android:title=\"uTube\" android:key=\"microg_group_key\" app:iconSpaceReserved=\"false\">\n        <Preference android:title=\"MicroG\"> <intent android:targetPackage=\"{playServicesName.Transformed}.android.gms\" android:targetClass=\"org.microg.gms.ui.SettingsActivity\"/> </Preference>\n        </PreferenceCategory>\n        </PreferenceScreen>")
                    ],
                    ...
                    (
                        ...
                        if (smaliProperties.Path.EndsWith("\\settings_fragment_cairo.xml"))
                        {
                            if (smaliSearchKeys.Any(st => smaliProperties.Full.Contains(st.Original)))
                            {
                                codeInject.FullReplace(
                                    [
                                        (("Experimental Features Settings Button Removal", true),
                                    ...
                                    }.All(smaliProperties.Lines[i].Contains))
                                {
                                    for (int j = i; j >= scaleIndex.Lines(i, -9); j--)
                                    {
                                            ...
                                            }.All(smaliProperties.Lines[j].Contains))
                                        {
                                            codeInject.Lines(
                                                [
                                                    (("Last Quality Layout Enabler", true),
                            ...
                            }.All(smaliProperties.Full.Contains))
                        {
                                        ...
                                        }.All(smaliProperties.Lines[i].Contains))
                                    {
                                        for (int j = i; j < smaliProperties.LinesCount; j++)
                                        {
                                                ...
                                                }.All(smaliProperties.Lines[j].Contains))
                                            {
                                                codeInject.Lines(
                                                    [
                                                        (("Miniplayer Enabler", true),

                                                        j - 1,

                                                        getPatch(0, smaliProperties.Lines[j].GetRegister(1), 1))
                                                    ]
                                                );
                            ...
                            if (patchInteractions.Equals(1)) {
                                for (int i = 0; i < smaliProperties.LinesCount; i++)
                                {
                                    ...
                                    {
                                        for (int j = i; j <= scaleIndex.Lines(i, 77); j++)
                                        {
                                            if (new[] {
                                                    smaliSearchKeys[6],
                                                    smaliSearchKeys[7]
                                                }.All(smaliProperties.Lines[j].Contains))
                                            {
                                                codeInject.Lines(
                                                    [
                                                        ...
                                                        getPatch(0, smaliProperties.Lines[j].GetRegister(1), 3 /* Miniplayer Type */))
                                                    ]
                                                );
                            ...
                            if (patchInteractions.Equals(2)) {
                                for (int i = 0; i < smaliProperties.LinesCount; i++)
                                {
                                    ...
                                    if (i.Equals(smaliProperties.LinesCount - 1))
                                    {
                                        codeInject.Write();
