...
namespace uDrop.Code
{
    public class APKUtils
    {
        ...
        private static readonly string integrationsSourcePath =
            $"AppIntegrations\\{Init.projectInfo.Item2.ToLower()}";
        public static readonly string logsDirName =
            "PatchingLogs";

        private static List<string> SmaliPaths = [];
        public static void SetSmaliPaths(string apkDecompiledPath)
        {
        ...
        public static bool Decompile()
        {
            ...
            startInfo = new()
            {
                WindowStyle = ProcessWindowStyle.Hidden,
                FileName = "cmd.exe",
                Arguments = $"/C java -jar {apktoolPath} d -f {Init.apkPath} -o {Init.apkDecompiledPath}",
                RedirectStandardOutput = true,
                RedirectStandardError = true
            };
            process.StartInfo = startInfo;
        ...
        public static void SmaliBalance()
        {
            ...
            List<string> dirPaths = SmaliUtils.GetSmaliFolders(Init.apkDecompiledPath);
                    ...
                    for (int i = 0; i < checkedAmountToMove; i++)
                    {
                        File.Move(
                            ...
                            $"{(!isLastExistingDir
                                ...
                                :
                                currentDirPath)}\\{Path.GetFileName(dirFiles[i])}"
                        );
                    }
                }
            ...
            SetSmaliPaths(Init.apkDecompiledPath);

            "Smali Folders succesfully balanced".EndProcessLog(true);
        }

        public static void AppPatches()
        {
            if (SmaliPaths.Count.Equals(0))
            {
                "Error: Smali files list is empty".QuitWithException();
            }

            string methodName;

            foreach (MethodInfo method in Activator
                                            .CreateInstance(
                                                null!,
                                                $"{MethodBase
                                                        .GetCurrentMethod()?
                                                        .DeclaringType?
                                                        .Namespace ?? string.Empty}.{Init.projectInfo.Item2}_Patches"
                                            )?
                                            .Unwrap()?
                                            .GetType()
                                            .GetMethods(
                                            ...
                                            )!)
            {
                ...
                if (excludedMethods.Any(methodName.Contains))
                {
                    continue;
                }
        ...
        {
            string[] integrationFiles = Directory.GetFiles(
                                            integrationsSourcePath,
                                            "*.*",
                                            SearchOption.AllDirectories
                                        );

            if (!Directory.Exists(integrationsSourcePath) || integrationFiles.Length.Equals(0))
            {
                return;
            }
            ...
            foreach (string integrationFile in integrationFiles)
            {
                integrationFileDestinationPath =
                    integrationFile.Replace(integrationsSourcePath, Init.apkDecompiledPath);

                Directory.CreateDirectory(Directory.GetParent(integrationFileDestinationPath)!.FullName);
                File.Copy(integrationFile, integrationFileDestinationPath, true);

                $"{integrationFileDestinationPath.GetSmaliFilePartialPath()} ---> Copied".LogFound(false);
            }
        ...
        public static void UnusedLibsRemoval()
        {
            ...
            string libDirPath = $"{Init.apkDecompiledPath}\\lib";
            ...
            foreach (string lib in new string[] { "armeabi-v7a", "x86" })
            {
        ...
        public static bool Compile()
        {
            ...
            ProcessStartInfo startInfo = new()
            {
                WindowStyle = ProcessWindowStyle.Hidden,
                FileName = "cmd.exe",
                Arguments = $"/C java -jar {apktoolPath} b {Init.apkDecompiledPath} -o {Init.apkModdedPath}",
                RedirectStandardOutput = true,
                RedirectStandardError = true
            };
            process.StartInfo = startInfo;
        ...
        public static void ZipAlign()
        {
            ...
            string alignedAPKPath = $"{Init.apkModdedPath}_aligned";

            if (File.Exists(Init.apkModdedPath))
            {
                File.Move(Init.apkModdedPath, alignedAPKPath, true);
            }
            ...
            ProcessStartInfo startInfo = new()
            {
                WindowStyle = ProcessWindowStyle.Hidden,
                FileName = "cmd.exe",
                Arguments = $"/C {zipalignPath} -p -f -v 4 {alignedAPKPath} {Init.apkModdedPath}",
                RedirectStandardOutput = true,
                RedirectStandardError = true
            };
            process.StartInfo = startInfo;
        ...
        public static void Sign()
        {
            ...
            string[] signKeyFilePaths = [
                                            ...
                                            $"{signKeyFolderName}/signKeyInfos.json"
                                        ];

            bool checkSignKeyFilesExists = false;

            if (signKeyFilePaths.All(File.Exists))
            {
                checkSignKeyFilesExists = true;
            }
            ...
            else
            {
                ...
                string[] randomSignDName = randomStrings[2..8]; //Owner Sign Key Infos
            ...
            if (checkSignKeyFilesExists)
            {
                string signKeyInfosData = File.ReadAllText(signKeyFilePaths[1]);
                dynamic signKeyInfosDeserialized = new ExpandoObject();
                if (!string.IsNullOrEmpty(signKeyInfosData))
                {
                    signKeyInfosDeserialized = JsonConvert.DeserializeObject(signKeyInfosData)!;

                    if (signKeyInfosDeserialized.Alias == null || signKeyInfosDeserialized.Pass == null)
                    {
                        ("Error: Alias or Pass is missing in 'signKeyInfos.json'\n" +
                        "Press any key to close the patcher.")
                            .QuitWithException();
                    }
                }
                ...
                {
                    ("Error: 'signKeyInfos.json' is empty\n" +
                    "Press any key to close the patcher.")
                        .QuitWithException();
                }
                ...
                startInfo = new()
                {
                    WindowStyle = ProcessWindowStyle.Hidden,
                    FileName = "cmd.exe",
                    Arguments = $"/C java -jar {apksignerPath} sign --ks {signKeyFilePaths[0]} --ks-key-alias {signKeyInfosDeserialized.Alias} --ks-pass pass:{signKeyInfosDeserialized.Pass} {Init.apkModdedPath}",
                    RedirectStandardOutput = true,
                    RedirectStandardError = true
                };
                process.StartInfo = startInfo;
        ...
        public static void TemporaryResourceRemoval()
        {
            ...
            if (Directory.Exists(Init.apkDecompiledPath))
            {
                Directory.Delete(Init.apkDecompiledPath, true);
            }
        ...
        public static void DoneMessage()
        {
            ...
            if (!patchingFailed)
            {
                ...
                Process.Start("explorer.exe", Init.secondCombinedRootFolders);
