...
import static uTools.StreamSpoofing.uPlayerRoutes.GET_STREAMING_DATA;
...
public class uStreamingDataRequest {
    ...
    static {
        ...
        uClientType preferredClientType = uClientType.ANDROID_VR;
    ...
    private static ByteBuffer fetch(String videoId, Map<String, String> playerHeaders) {
        clientTypes: for (uClientType clientType : CLIENT_TYPES_ORDER_TO_USE) {
                    ...
                    if (contentLength != 0) {
                        try (
                            ...
                            while ((bytesRead = inputStream.read(buffer)) >= 0) {
                                if (isLiveStream(new String(
                                        buffer,
                                        0,
                                        Math.min(bytesRead, 512)
                                    ))
                                    &&
                                    !clientType.name().equals("ANDROID_VR")) {
                                        continue clientTypes;
                                }
