...
public class uStreamingDataRequest {
    ...
    private static final uClientType LIVE_STREAM_CLIENT = uClientType.ANDROID_VR;
    static {
        for (uClientType clientType : uClientType.values()) {
    ...
    private static ByteBuffer Fetch(String videoID, Map<String, String> playerHeaders) {
        moveToNextClient: for (uClientType clientType : CLIENT_TYPES_ORDER_TO_USE) {
                    ...
                    if (contentLength != 0) {
                        try (
                            ...
                            while ((bytesRead = inputStream.read(buffer)) >= 0) {
                                if (IsLiveStream(buffer, bytesRead) &&
                                    clientType != LIVE_STREAM_CLIENT) {
                                        continue moveToNextClient;
                                }
    ...
    private static boolean IsLiveStream(byte[] buffer, int bytesRead) {
        return Stream.of(
