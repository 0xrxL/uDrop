...
public class uBlocker {
    ...
    public static void HideActionButtons(List<Object> list, String identifier) {
        ...
        if (identifier != null &&
            ...
            int listSize =
                list.size();
            AtomicInteger currentActionButtonsListSize =
                new AtomicInteger(currentActionButtonsList.size());

            int nextToActionShareButtonIndex =
                IntStream.range(0, currentActionButtonsListSize.get())
                .filter(i -> currentActionButtonsList.get(i).contains(visibleActionButtons.get(0)))
                .findFirst()
                .orElse(-1);

            if (nextToActionShareButtonIndex > -1) {
                nextToActionShareButtonIndex++;

                if (listSize - currentActionButtonsListSize.get() >= 1) {
                    ...
                    currentActionButtonsListSize.getAndIncrement();
                }
                if (listSize - currentActionButtonsListSize.get() == 1) {
                    currentActionButtonsList.add(
                        nextToActionShareButtonIndex,
                ...
                if (currentActionButtonsListSize.get() - listSize == 1) {
                    currentActionButtonsList.removeIf(
                        str
                            ->
                        {
                            if (str.contains(visibleActionButtons.get(0))) {
                                currentActionButtonsListSize.getAndDecrement();
                ...
                if (currentActionButtonsListSize.get() != listSize) {
                    Log.e(
                        GetClassName(),

                        String.format(
                            "Action button filtering list doesn't contains all original objects:\nFiltering: %s\nOriginal: %s",

                            currentActionButtonsListSize.get(),
                            listSize
                        )
                    );
    ...
    private static final Set<String> commentsComponentsThird =
        Set.of(
            "yt_outline_bookmark",
            "yt_outline_flag"
        );
